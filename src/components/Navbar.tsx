        ...
        <div className="max-w-screen-xl mx-auto py-6 flex items-center justify-between border-b-[1px] border-zinc-700¸">
            ...
            <div className="flex gap-10 ml-10">
                {['Home', 'Wrok', 'Culture', '', 'News'].map((elem,index) => (
                    elem.length === 0 ? <span className="w-[2px] h-5 bg-zinc-500"></span> : <a className="text-sm gap-1 flex items-center">{index === 1 && ( <span style={{boxShadow: "0 0 0.45em #00FF19"}}className="rounded-full w-1 h-1 bg-green-500"></span>)}{elem}</a>))} 
            </div>
            </div>
            <Button />
        </div>
